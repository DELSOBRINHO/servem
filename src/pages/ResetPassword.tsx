...
const ResetPassword: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (resetError) {
        error(resetError.message || 'Erro ao solicitar redefinição de senha.');
        return;
      }
      
      success('Instruções para redefinição de senha foram enviadas para seu e-mail.');
      setSubmitted(true);
    } catch (err) {
        ...
        {submitted ? (
          ...
          <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
            ...
            <div className="text-center">
              <Link to="/login" className="font-medium text-servem-primary hover:text-indigo-500">
                Voltar para o login
              </Link>
            </div>
          </form>
